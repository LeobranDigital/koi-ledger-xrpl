...
def open_seller_wallet():
    win = tk.Tk()  # Use Tk() for main window
    win.title("Seller Wallet")
    win.geometry("800x450")

    tk.Label(win, text="Seller XRP Wallet", font=("Arial", 16)).pack(pady=5)
    ...
    def refresh_balance():
        try:
            resp = client.request(AccountInfo(account=SELLER_ADDRESS, ledger_index="validated"))
    ...
    def load_transactions():
        def fetch():
            try:
                ...
                resp = client.request(AccountTx(account=SELLER_ADDRESS, limit=10))
